...
% competence VS performance (Phillips: what's performance?); grammar as abstraction of parser? what's the divide?
% why do we care about this in the first place?
    % - model human cognition; explain processing quirks (which are surprising!), some of them being:
        % - grammaticality illusions (NPIs, number agreement; but not person agreement)
        % - ungrammaticality illusions (garden path sentences)
        % - structure sensitivity (SC/RC vs RC/SC; center embedding VS right embedding)
        % - structure insensitivity (merely local syntactic coherence effects)
        % - word order effects
        % - lack of word order effects (SRC over ORC irrespective of word order)
        % - language-specific variation (attachment preferences)
    % - build human-like machines (e.g. to seem more human, pass the Turing test by replicating our failures); at the very least: only produce easily parseable sentences
    % - improve efficiency of industrial parsers by learning from nature (lotus effect)
    % - use processing data as evidence in syntactic analysis (cf. DTC)
    % - use parser as restriction on UG (only these grammars are efficiently parseable)
