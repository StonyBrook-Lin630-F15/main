...
\begin{examplebox}[LC Parse of Nested Possessive DPs]
    The table below shows a successful parse for \emph{John's father's car's exhaust pipe disappeared} with a grammar that analyzes possessive DPs with the rules DP \rewrite\ DP D$'$ and D$'$ \rewrite\ Poss NP\@.
    %
    \begin{center}
        \begin{tabular}{r|l}
            \textbf{parse item} & \textbf{inference rule}\\
